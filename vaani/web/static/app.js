...
document.addEventListener("DOMContentLoaded", () => {
  ...
  if (navPostBtn) {
    navPostBtn.addEventListener("click", () => {
      ...
      setTimeout(() => {
        ...
        modalReplyInput.setSelectionRange(modalReplyInput.value.length, modalReplyInput.value.length);
      }, 50);
    });
  }
  ...
  // ============================================================
  // Helper: Append a Full Thread to Timeline
  // ============================================================
  ...
  // ============================================================
  // REPLY MODAL (Comment / Reply on any tweet)
  // ============================================================
  const replyModal = document.getElementById("reply-modal");
  const modalCloseBtn = document.getElementById("modal-close-btn");
  const modalOrigName = document.getElementById("modal-orig-name");
  const modalOrigHandle = document.getElementById("modal-orig-handle");
  const modalOrigText = document.getElementById("modal-orig-text");
  const modalOrigAvatar = document.getElementById("modal-orig-avatar");
  const modalReplyingMention = document.getElementById("modal-replying-mention");
  ...
  let activeReplyParentThread = null;

  // Click on reply button on any tweet opens the comment modal
  tweetFeed.addEventListener("click", (e) => {
    const replyBtn = e.target.closest(".btn-reply");
    if (replyBtn) {
      const tweetRow = replyBtn.closest(".tweet-row");
      const parentThread = replyBtn.closest(".tweet-thread-block");
      if (!tweetRow) return;

      const authorName = tweetRow.querySelector(".t-author-name")?.textContent || "user";
      const authorHandle = tweetRow.querySelector(".t-author-handle")?.textContent || "@user";
      const tweetText = tweetRow.querySelector(".tweet-text")?.textContent || "";
      const avatarText = tweetRow.querySelector(".avatar-img")?.textContent || "U";

      modalOrigName.textContent = authorName;
      modalOrigHandle.textContent = authorHandle;
      modalOrigText.textContent = tweetText;
      modalOrigAvatar.textContent = avatarText;
      modalReplyingMention.textContent = authorHandle;
  ...
  function updateModalCharCount() {
    const len = modalReplyInput.value.length;
    modalCharCount.textContent = `${len} / 280`;
    modalCharCount.style.color = len > 280 ? "#f4212e" : "var(--text-sub)";
    modalSubmitBtn.disabled = len === 0 || len > 280;
  }

  modalReplyInput.addEventListener("input", updateModalCharCount);
  ...
  async function handleModalReplySubmit() {
    ...
    // Close modal immediately for smooth Twitter UX
    replyModal.style.display = "none";
    ...
    if (!targetThread) {
      ...
      targetThread.className = "tweet-thread-block";
      tweetFeed.prepend(targetThread);
    }

    // 1. User reply row
    const userReplyRow = document.createElement("div");
    userReplyRow.className = "tweet-row";
    ...
    try {
      const response = await fetch("/api/mention", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ text: replyText, author: "samarthsharma44" })
      });
      ...
      data.chunks.forEach((chunk, idx) => {
        ...
        `;
        targetThread.appendChild(botRow);
      });
